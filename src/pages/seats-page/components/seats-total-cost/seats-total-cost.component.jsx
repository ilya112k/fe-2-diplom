...
import "./seats-total-cost.component.css";

function SeatsTotalCostComponent({ identity }) {
    ...
    const ticketsDepCost = orderState.departure && orderState.departure.seats ? orderState.departure.seats.reduce((acc, item) => acc + (item.seat_cost || 0), 0) : null;
    const ticketsArrCost = orderState.arrival && orderState.arrival.seats ? orderState.arrival.seats.reduce((acc, item) => acc + (item.seat_cost || 0), 0) : null;
...
export default SeatsTotalCostComponent;

SeatsTotalCostComponent.propTypes = {
